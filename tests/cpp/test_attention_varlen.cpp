...
TEST(AttentionVarlenNeon, MatchesRefAcrossEsmHeadDims) {
  // ESM head_dim 16/24/32/64 plus odd dh (3, 17) and B=2 packed isolation.
  struct Case { int H, dh, L1, L2; };
  const Case cases[] = {
      {2, 3, 5, 0},    {4, 16, 32, 0},  {4, 17, 9, 0},   {4, 24, 16, 0},
      {4, 32, 17, 0},  {4, 64, 33, 0},  {4, 16, 17, 23}, {4, 64, 11, 31},
      {20, 64, 256, 0}, {20, 64, 128, 128}, {20, 24, 37, 100}, {20, 64, 1, 1},
  };
  for (const auto& c : cases) {
    const int T = c.L1 + c.L2;
    auto Q = RandomVec(static_cast<std::size_t>(T) * c.H * c.dh, 0xA1A2);
    ...
    auto V = RandomVec(static_cast<std::size_t>(T) * c.H * c.dh, 0xC1C2);
    const float q_scale = 1.0f / std::sqrt(static_cast<float>(c.dh));
    for (auto& x : Q) x *= q_scale;
    std::vector<int> cu = c.L2 > 0 ? std::vector<int>{0, c.L1, T}
                                   : std::vector<int>{0, c.L1};
    const int batch_size = static_cast<int>(cu.size()) - 1;
    std::vector<float> out_ref(static_cast<std::size_t>(T) * c.H * c.dh);
    std::vector<float> out_neon(static_cast<std::size_t>(T) * c.H * c.dh);
    esm::kernels::AttentionVarlenRef(Q.data(), K.data(), V.data(), cu.data(),
                                     batch_size, c.H, c.dh, out_ref.data());
    esm::kernels::AttentionVarlenNeon(Q.data(), K.data(), V.data(), cu.data(),
                                      batch_size, c.H, c.dh, out_neon.data());
    // FP32 softmax accumulator vs the FP64-sum reference: ~1e-5 relative.
    for (std::size_t i = 0; i < out_ref.size(); ++i) {
      const float ref_mag = std::fabs(out_ref[i]);
